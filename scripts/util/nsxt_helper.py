...
def grabNsxtHeaders(jsonspec):
    try:
        str_enc = str(jsonspec['envSpec']['vcenterDetails']["nsxtUserPasswordBase64"])
        base64_bytes = str_enc.encode('ascii')
        enc_bytes = base64.b64decode(base64_bytes)
        password = enc_bytes.decode('ascii').rstrip("\n")

        ecod_bytes = (jsonspec['envSpec']['vcenterDetails']["nsxtUser"] + ":" + password).encode(
            "ascii")
        ecod_bytes = base64.b64encode(ecod_bytes)
        address = str(jsonspec['envSpec']['vcenterDetails']["nsxtAddress"])
...
def getTier1Details(headers_, jsonspec):
    uri = "https://" + headers_[2] + "/policy/api/v1/infra/tier-1s"
    response = requests.request(
        "GET", uri, headers=headers_[1], verify=False)
    if response.status_code != 200:
        return None, response.status_code
    teir1name = str(jsonspec['envSpec']['vcenterDetails']["nsxtTier1RouterDisplayName"])
...
def getESXIips(jsonspec):
    try:
        ...
        ecod_bytes = (jsonspec['envSpec']['vcenterDetails'][
                          "vcenterSsoUser"] + ":" + password).encode(
            "ascii")
        ecod_bytes = base64.b64encode(ecod_bytes)
        address = str(jsonspec['envSpec']['vcenterDetails']["vcenterAddress"])
        ecod_string = ecod_bytes.decode("ascii")
