...
class MgmtClusterWorkflow:
    ...
    def _template_deploy_yaml(self):
        ...
        FileHelper.write_to_file(
            t.render(config=self.run_config, avi_cert=get_base64_cert(self.run_config.spec.avi.fqdn),
    ...
    def _pre_validate(self):
        if self.run_config.state.mgmt.deployed:
            logger.info("Management cluster is deployed, and in version : %s", self.run_config.state.mgmt.version)
            if self.run_config.state.mgmt.version == self.run_config.desired_state.version.tkg:
                logger.info("Management cluster is already on correct version(%s)", self.run_config.state.mgmt.version)
                exit(0)
            if self.run_config.state.mgmt.version not in self.run_config.support_matrix["matrix"].keys():
                raise ValueError(f"Current Tanzu version({self.run_config.state.mgmt.version}) is unsupported")
            ...
            if self.run_config.desired_state.version.tkg not in self.run_config.support_matrix["upgrade_path"].get(
                ...
                raise ValueError(f"There are no upgrade path available for tkg: {self.run_config.state.mgmt.version}")
    ...
    def _check_management_cluster_exists(self, cluster_name):
        try:
            ...
            return False
        mgmt_clu = [c for c in cluster_list if Constants.MANAGEMENT_ROLE in c["roles"] and c["name"] == cluster_name]
        return any(mgmt_clu)
    ...
    def _update_state(self, msg="Successful management cluster deployment"):
        ...
        self.run_config.state.mgmt = Info(
            deployed=True, health=HealthEnum.UP, version=self.run_config.desired_state.version.tkg,
    ...
    def _register_tmc(self, ssh: SshHelper):
        ...
        ssh.run_cmd(
            TKGCommands.REGISTER_TMC.format(url=self.run_config.spec.tkg.common.tmcRegistrationUrl),
    ...
    def upgrade_workflow(self):
        ...
        with SshHelper(self.bootstrap.server, self.bootstrap.username,
            ...
            ClusterCommonWorkflow(ssh).commit_kubeconfig(self.run_config.root_dir, "management")
        self._update_state(f"Successful management cluster upgrade [{self.cluster_name}]")
