...
class deploy_tkg_extensions():
    ...
    def extention_types_dot4_impl(self, extensions_types):
    ...
    def deploy(self):
        dot4 = self.extention_types_dot4_impl()
        
        if str(self.extension_name).__contains__("Fluent"):
            status = dot4.fluent_bit(self.extention_name)
            return status[0], status[1]
        elif str(self.extention_name) == Tkg_Extention_names.GRAFANA:            
            status = dot4.grafana()
            return status[0], status[1]
        elif str(self.extention_name) == Tkg_Extention_names.LOGGING:
            status = dot4.logging()
            return status[0], status[1]
        elif str(self.extention_name) == Tkg_Extention_names.PROMETHEUS:
